...
-- Xóa hợp đồng sẽ xóa các bảng liên quan tới nó
CREATE OR REPLACE TRIGGER QLHSUT.BEFORE_XOA_HOP_DONG
BEFORE DELETE ON QLHSUT.QLHSUT_HOP_DONG_DANG_TUYEN
FOR EACH ROW
DECLARE
    v_hopdong_count NUMBER;
BEGIN
    DELETE FROM QLHSUT.QLHSUT_PHIEU_QUANG_CAO WHERE MAHOPDONG = :OLD.MAHOPDONG;
    DELETE FROM QLHSUT.QLHSUT_HINH_THUC_LUA_CHON WHERE MAHOPDONG = :OLD.MAHOPDONG;
END;
/

CREATE OR REPLACE TRIGGER QLHSUT.AFTER_XOA_HOP_DONG
AFTER DELETE ON QLHSUT.QLHSUT_HOP_DONG_DANG_TUYEN
FOR EACH ROW
BEGIN
    DELETE FROM QLHSUT.QLHSUT_THONG_TIN_DANG_TUYEN WHERE MADT = :OLD.MADT;
END;
/