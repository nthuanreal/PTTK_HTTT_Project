...
BEGIN
    ...
    IF USR_ROLE = N'Doanh nghiệp' THEN
        SELECT COUNT(*) INTO LaDN FROM QLHSUT.QLHSUT_DOANH_NGHIEP WHERE TO_CHAR(MADN) = USRNAME AND TTTAIKHOAN = 1;
    END IF;
...
AS
    ...
    V_COUNT NUMBER;
BEGIN     
    SELECT MAX(MADT) INTO V_MADT FROM QLHSUT.QLHSUT_THONG_TIN_DANG_TUYEN;
    V_MADT := V_MADT + 1;
...
AS
    ...
    V_COUNT NUMBER;
BEGIN     
    SELECT MAX(MAHOPDONG) INTO V_MAHD FROM QLHSUT.QLHSUT_HOP_DONG_DANG_TUYEN;
    V_MAHD := V_MAHD + 1;
...
AS
    ...
    V_COUNT NUMBER;
BEGIN     
    SELECT MAX(MAPQC) INTO V_MAPQC FROM QLHSUT.QLHSUT_PHIEU_QUANG_CAO;
    V_MAPQC := V_MAPQC + 1;
